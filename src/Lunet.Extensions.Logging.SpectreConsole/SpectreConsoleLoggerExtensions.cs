...
/// <summary>
/// ILogger extension methods for common scenarios by adding markup logging.
/// </summary>
public static class SpectreConsoleLoggerExtensions
{
    /// <summary>
    /// Add spectre console to <see cref="ILoggingBuilder"/>.
    /// </summary>
    /// <param name="loggingBuilder">The logging builder.</param>
    /// <param name="options">Spectre console logger options.</param>
    /// <returns>The logging builder.</returns>
    public static ILoggingBuilder AddSpectreConsole(this ILoggingBuilder loggingBuilder, SpectreConsoleLoggerOptions? options = null)
    {
        options ??= new SpectreConsoleLoggerOptions();
        loggingBuilder.AddProvider(new SpectreConsoleLoggerProvider(options));
    ...
    public static void LogMarkup(this ILogger logger, LogLevel logLevel, EventId eventId, Exception? exception, string? message, params object?[] args)
    {
        ...
        try
        {
            ...
            var renderArgs = args.Length == 0 ? Array.Empty<IRenderable>() : args.Select(x => x is IRenderable renderable ? renderable : new Text(x?.ToString() ?? string.Empty)).ToArray();
            logger.Log(logLevel, eventId, new SpectreConsoleLoggerMessage(message, renderArgs), exception, (renderable, exceptionToFormat) =>
            {
                var console = SpectreConsoleLoggerRenderer.CreateAsciiOffScreenConsole();
                return SpectreConsoleLoggerRenderer.Render(console, renderable, exception);
            });
        }
        finally
        {
        ...
        params object?[] args)
    {
        ...
        try
        {
            SpectreConsoleLogger.EnableMarkup = true;
            var renderArgs = args.Length == 0 ? Array.Empty<IRenderable>() : args.Select(x => x is IRenderable renderable ? renderable : new Text(x?.ToString() ?? string.Empty)).ToArray();
            logger.BeginScope(new SpectreConsoleLoggerMessage(messageFormat, renderArgs));
